...
# Taken and modified from: https://github.com/facebookincubator/buck2/blob/main/shim/third-party/macros/rust_third_party.bzl
...
def _make_cmd(mode, buildscript, package_name, version, features, cfgs, env, target_override):
    flags = [
        ...
        ("cfg", cfgs),
        ("env", env),
        ("target", target_override or get_native_host_triple()),
    ]

    cmd = "$(exe root//third_party:build_rs)"
